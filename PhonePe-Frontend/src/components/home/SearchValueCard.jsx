...
function SearchValueCard({ 
    ...
    const handleUserClick = (user) => {
        const fromMerchantId = localStorage.getItem('merchantId');
        if (!fromMerchantId) {
            console.error('No merchant ID found in localStorage. Please log in again.');
            return;
        }

        if (!user.merchantId) {
            console.error('Selected user has no merchant ID:', user);
            return;
        }
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg">
            ...
            <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 mb-6">
                ...
                <input
                    ...
                    className="flex-1 bg-gray-50 border border-gray-300 text-[#b0a0db]-900 rounded-lg px-4 py-2 focus:ring-blue-500 focus:border-blue-500"
                />
                <button
